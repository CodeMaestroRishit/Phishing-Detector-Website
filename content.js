...
const MIN_ANALYSIS_CHARS = 400; // Minimum chars for reliable analysis
...
(function ensureStyles() {
  ...
  style.textContent = `
    ...
    /* Insufficient context variant */
    .phish-detector-tooltip.low-context {
      background: linear-gradient(135deg, #1e40af 0%, #1e3a8a 100%);
      border-color: rgba(255,255,255,0.2);
    }
    
    .phish-detector-tooltip.low-context .icon {
      background: rgba(255,255,255,0.2);
      color: #ffffff;
    }
  `;
...
async function classifyText(text) {
  if (text.length < MIN_ANALYSIS_CHARS) {
    return { 
      verdict: "low_context", 
      message: "⚠️ Insufficient context. Unable to determine. Scanning full email recommended.",
      probability: 0
    };
  }
  ...
  try {
    ...
    return { 
      verdict: label === 1 ? "phishing" : "safe",
      message: label === 1 ? "Phishing detected!" : "Content appears safe",
      probability: prob
    };
  } catch (e) {
    return { 
...
async function analyzeSelectionAtPoint(clientX, clientY, text) {
  ...
  if (result.verdict === "low_context") {
    state = {
      variant: "low-context",
      icon: "ℹ",
      title: "Insufficient Context",
      message: "Unable to determine. Scanning full email recommended.",
      probability: 0,
      footer: "Select more text (at least 400 characters) for reliable analysis.",
      duration: 5000
    };
    ...
    state = {
      ...
      message: result.message,
      probability: 1 - result.probability,
